  ...
  switch (type) {
    ...
    case Errors.DELETE_MESSAGES_MIN:
      return await message
        .send("You need to provide atleast 2 messages to delete multiple messages at once.")
        .catch(log.error);
    ...
    case Errors.PRUNE_MIN_DAYS:
      return await message
        .send("You can not prune members from the server with less than 1 days activity requirement.")
        .catch(log.error);
    case Errors.RATE_LIMIT_RETRY_MAXED:
      return await message
        .send("Errored more than the maximum amount of retries. Please contact my developers to have this fixed.")
        .catch(log.error);
    case Errors.MISSING_INTENT_GUILD_MEMBERS:
      return await message
        .send("Unable to fetch members if the bot does not have the GUILD MEMBERS intent.")
        .catch(log.error);
    ...
    case Errors.CHANNEL_NOT_IN_GUILD:
      return await message
        .send("This function is only able to be done inside a server. This channel was not found in any server.")
        .catch(log.error);
    default:
      return;
  }
