...
bot.eventHandlers.ready = async function () {
  ...
  console.log(getTime(), `Loaded ${Object.keys(bot.eventHandlers).length} Event(s)`);
  ...
  console.log(getTime(), `[READY] Bot is online and ready in ${cache.guilds.size} guild(s)!`);
  ...
  await Promise.all(
    cache.guilds.map(async (guild) => {
      ...
      console.log(getTime(), `Updated Guild ${guild.name} (${guild.id}) Slash Commands...`);
    })
  );
