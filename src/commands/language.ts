...
createCommand({
  ...
  execute: function (message) {
    ...
    const currentLanguage = allowedLanguages.find((item) => item.id === currentLanguageId) || allowedLanguages[0];
  ...
  arguments: [
    {
      ...
      missing: function (message) {
        const embed = new Embed().setTitle("Available Languages").setDescription(listOfLanguages);
        message.send({ embed });
      },
    },
  ] as const,
