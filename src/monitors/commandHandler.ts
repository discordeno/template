...
export const commandHandler = async (message: Message) => {
  ...
  if (inhibitor_results.includes(true)) return logCommand(message, guild?.name || "DM", "Inhibibted")
...
export const logCommand = (message: Message, guild_name: string, type: string) => {
